...
if (isset($_GET['ajax']) && $_GET['ajax'] == '1') {
    if (!empty($customers)) {
        foreach ($customers as $customer) {
            ...
            echo '<td>' . htmlspecialchars($customer['customer_city'] ?? '-') . '</td>';
            echo '<td>' . htmlspecialchars($customer['customer_region'] ?? '-') . '</td>';
            echo '<td>' . htmlspecialchars($customer['customer_postcode'] ?? '-') . '</td>';
            echo '<td>' . htmlspecialchars($customer['customer_country'] ?? '-') . '</td>';
            echo '<td><a href="customer-edit-update.php?id=' . urlencode($customer['customer_id']) . '" class="btn btn-sm btn-primary" onclick="localStorage.setItem(\'customerFromEdit\', \'true\');">Edit</a></td>';
            echo '</tr>';
        }
    } else {
                                                        ...
                                                        <?php if (!empty($customers)): ?>
                                                            <?php foreach ($customers as $customer): ?>
                                                                <tr>
                                                                    ...
                                                                    <td><a href="customer-edit-update.php?id=<?= urlencode($customer['customer_id']) ?>" class="btn btn-sm btn-primary" onclick="localStorage.setItem('customerFromEdit', 'true');">Edit</a></td>
                                                                </tr>
                                                            <?php endforeach; ?>
                                                        <?php else: ?>
...
<script>
// Check if we should restore search (only if coming from edit page)
function shouldRestoreSearch() {
    const fromEdit = localStorage.getItem('customerFromEdit');
    if (fromEdit === 'true') {
...
function saveSearchState() {
    ...
    localStorage.setItem('customerSearchState', searchValue);
}

// Restore search state on page load
function restoreSearchState() {
    if (!shouldRestoreSearch()) {
        localStorage.removeItem('customerSearchState');
        return;
    }
    
    const savedSearch = localStorage.getItem('customerSearchState');
    if (savedSearch) {
        const searchInput = document.getElementById('searchInput');
        if (searchInput && searchInput.value === '') {
            searchInput.value = savedSearch;
            // Trigger the search to actually filter the data
            liveSearch();
        }
    }
}

// Set flag when clicking edit links
...
function liveSearch() {
    ...
    searchTimer = setTimeout(() => {
        ...
        saveSearchState(); // Save search when user types
        ...
        fetch(url.toString())
            ...
            .then(html => {
                ...
                // Re-attach onclick handlers to new Edit buttons
                document.querySelectorAll('a[href*="customer-edit-update.php"]').forEach(link => {
                    link.onclick = setEditFlag;
                });
            })
...
window.addEventListener('DOMContentLoaded', () => {
    restoreSearchState();
    // Attach onclick to all Edit buttons
    document.querySelectorAll('a[href*="customer-edit-update.php"]').forEach(link => {
        link.onclick = setEditFlag;
    });
});
