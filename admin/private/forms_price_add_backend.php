...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        // Check if a price record for this product already exists
        $stmtCheck = $pdo->prepare("SELECT price_id FROM price WHERE product_id = ?");
        $stmtCheck->execute([$product_id]);
        $existingPriceId = $stmtCheck->fetchColumn();

        if ($existingPriceId) {
            // Duplicate found - set error and skip insert
            $errorMsg = "❌ A price record already exists for the selected product (Price ID: $existingPriceId). Please edit it instead.";
        } else {
            // Proceed with insert
            $sql = "
            INSERT INTO price (
                ...
                final_profit_per_unit_rm, final_total_profit, final_profit_percent, zakat, final_selling_unit, weight_carton
            ) VALUES (
                :product_id, :supplier_id, :quantity,
                :carton_width, :carton_height, :carton_length, :pcs_per_carton, :no_of_carton,
                :designlogo, :price, :shipping_price, :additional_price, :conversion_rate, :price_rm,
                ...
                :final_selling_total, :final_total_price, :final_unit_price,
                :final_profit_per_unit_rm, :final_total_profit, :final_profit_percent, :zakat, :final_selling_unit,
                :weight_carton
            )";
            ...
            $bind[':zakat']                    = floatval($_POST['zakat'] ?? 0);
